...
void LandoltNodelet::onInit() {
  ...
  /* private_nh.param("camera_picoflexx_left", camera_reading_, std::string("/camera_3d/rgb/image_raw")); */
  /* private_nh.param("camera_reading", camera_reading_, std::string("/camera_3d/rgb/image_raw")); */
  /* private_nh.param("camera_reading", camera_reading_, std::string("/camera_3d/rgb/image_raw")); */
  /* private_nh.param("camera_reading", camera_reading_, std::string("/camera_3d/rgb/image_raw")); */
  /* private_nh.param("camera_reading", camera_reading_, std::string("/camera_3d/rgb/image_raw")); */
...
void LandoltNodelet::imageCb(const sensor_msgs::ImageConstPtr& image_msg, const sensor_msgs::CameraInfoConstPtr& info_msg) {
  ...
  try {
    img_ptr = cv_bridge::toCvShare(image_msg, sensor_msgs::image_encodings::MONO16);
  }
  ...
  blur(img_ptr->image, img_ptr->image, cv::Size(3, 3));                    // apply blur to grayscaled image
  threshold(img_ptr->image, img_ptr->image, 100, 255, cv::THRESH_BINARY);  // apply binary thresholding
  ...
  if (image_pub_.getNumSubscribers() > 0) {
    cv_bridge::CvImage img_msg(header, sensor_msgs::image_encodings::MONO16, img_ptr->image);
...
void LandoltNodelet::findLandoltGaps(const cv::Mat& imageRaw, Gaps& gaps, int minEdge, float minRatioCircle, int minDepth) {
  cv::Mat thresholdMat;
  cvtColor(thresholdMat, thresholdMat, cv::COLOR_BGR2GRAY);                // convert to grayscale
  blur(imageRaw, thresholdMat, cv::Size(3, 3));                    // apply blur to grayscaled image
  threshold(imageRaw, thresholdMat, 140, 255, cv::THRESH_BINARY);  // apply binary thresholding

  ROS_INFO("[findLandoltGaps]: preprocessing video");
  ...
  ROS_INFO("[findLandoltGaps]: find contours");
